...
(function () {
    ...
    function showOverview() {
        ...
        archHeader.innerHTML = `${t("architectureInfo")}: <span style="color: ${is64Bit ? "#4CAF50" : "#2196F3"}; font-weight: bold;">${bitInfo} ${t("bitPEFile")}</span>`;
        container.appendChild(archHeader);

        // DOS头信息
        if (parsedData.dos_header) {
            const dosRows = [
                [t("magic"), String(parsedData.dos_header.e_magic || "N/A"), `0x${(parsedData.dos_header.e_magic || 0).toString(16).toUpperCase()}`, t("mzIdentifier")],
                [t("ntHeaderOffset"), String(parsedData.dos_header.e_lfanew || "N/A"), `0x${(parsedData.dos_header.e_lfanew || 0).toString(16).toUpperCase()}`, t("ntHeaderPosition")],
            ];
            container.appendChild(createTable(t("dosHeaderInfo"), [t("field"), t("value"), t("hex"), t("description")], dosRows, ["", "pe-details-value", "pe-details-hex", ""]));
        }
        ...
        if (parsedData.nt_headers) {
            ...
            if (parsedData.nt_headers.OptionalHeader) {
                ntRows.push([t("addressOfEntryPoint"), String(parsedData.nt_headers.OptionalHeader.AddressOfEntryPoint || "N/A"), formatAddress(parsedData.nt_headers.OptionalHeader.AddressOfEntryPoint || 0), t("entryPointAddress")]);
                ntRows.push([t("imageBase"), String(parsedData.nt_headers.OptionalHeader.ImageBase || "N/A"), formatAddress(parsedData.nt_headers.OptionalHeader.ImageBase || 0), t("imageBaseAddress")]);
            }
            container.appendChild(createTable(t("ntHeaderInfo"), [t("field"), t("value"), t("hex"), t("description")], ntRows, ["", "pe-details-value", "pe-details-hex", ""]));
        }
        ...
        if (parsedData.sections && parsedData.sections.length > 0) {
            const sectionRows = parsedData.sections.map((section, index) => {
                ...
                return [sectionName, String(section.VirtualSize || "N/A"), formatAddress(section.VirtualAddress || 0), String(section.SizeOfRawData || "N/A"), formatAddress(section.PointerToRawData || 0), `0x${(section.Characteristics || 0).toString(16).toUpperCase()}`];
            });
        ...
        if (parsedData.exports && parsedData.exports.functions && parsedData.exports.functions.length > 0) {
            ...
            const exportRows = [["导出函数数量", String(exportCount), "", '点击左侧"导出函数"查看详细列表']];
    ...
    function showDataDirectory() {
        ...
        const directoryNames = [t("exportTable"), t("importTable"), t("resourceTable"), t("exceptionTable"), t("certificateTable"), t("relocationTable"), t("debugInfo"), t("architectureData"), t("globalPointer"), t("tlsTable"), t("loadConfigTable"), t("boundImportTable"), t("iat"), t("delayImportTable"), t("clrRuntimeHeader"), t("reserved")];

        /** @type {Array<string[]>} */
        const dirRows = [];
        dataDir.forEach((/** @type {any} */ dir, /** @type {number} */ index) => {
            if (dir && (dir.VirtualAddress || dir.Size)) {
                dirRows.push([String(index), directoryNames[index] || t("unknown"), formatAddress(dir.VirtualAddress || 0), String(dir.Size || 0)]);
            }
        });

        peDetails.appendChild(createTable(t("dataDirectoryTable"), [t("index"), t("type"), t("virtualAddress"), t("size")], dirRows, ["pe-details-value", "pe-details-value", "pe-details-hex", "pe-details-value"]));
    }
    ...
    function showAllSections() {
        ...
        const sectionRows = parsedData.sections.map((/** @type {Section} */ section, /** @type {number} */ index) => {
            const sectionName = section.Name ? section.Name.replace(/\0/g, "") : `Section ${index + 1}`;
            return [sectionName, formatAddress(section.VirtualAddress || 0), String(section.VirtualSize || 0), formatAddress(section.PointerToRawData || 0), String(section.SizeOfRawData || 0), `0x${(section.Characteristics || 0).toString(16).toUpperCase()}`];
        });

        peDetails.appendChild(createTable(t("allSectionsOverview"), [t("sectionName"), t("virtualAddress"), t("virtualSize"), t("rawPointer"), t("rawSize"), t("characteristics")], sectionRows, ["pe-details-value", "pe-details-hex", "pe-details-value", "pe-details-hex", "pe-details-value", "pe-details-hex"]));
    }
    ...
    function showSection(sectionName) {
        ...
        const header = document.createElement("h4");
        header.textContent = t("sectionDetails").replace("{sectionName}", sectionName);
        container.appendChild(header);
    ...
    function showDllImports(dll) {
        ...
        const funcCount = dll.functions ? dll.functions.length : 0;
        detailsTitle.textContent = `${dll.name} - ${t("importedFunctionsTitle").replace("{dllName}", dll.name)} (${t("importedFunctionsCount").replace("{count}", funcCount)})`;
        peDetails.innerHTML = "";
        ...
        const funcRows = dll.functions.map((/** @type {ImportFunction} */ func) => {
            return [dll.name, func.name || `${t("ordinalPrefix")}${func.ordinal}`, func.name ? t("byName") : t("byOrdinal")];
        });
    ...
    function generateValueDetails(value, path) {
        ...
        if (typeof value === "number" || typeof value === "bigint") {
            ...
            container.appendChild(createTable(t("numericDetails"), [t("property"), t("value")], rows, ["", "pe-details-value"]));
            ...
            container.appendChild(createTable(t("stringDetails"), [t("property"), t("value")], rows, ["", "pe-details-value"]));
            ...
            for (const [key, val] of Object.entries(value)) {
                if (typeof val === "number" || typeof val === "bigint") {
                    ...
                    rows.push([key, t("arrayWithLength").replace("{length}", val.length), "-"]);
            ...
            container.appendChild(createTable(t("structureDetails"), [t("field"), t("value"), t("hexadecimal")], rows, ["", "pe-details-value", "pe-details-hex"]));
        } else {
    ...
    function showResourcesOverview() {
        ...
        // 查找.rsrc资源节
        const rsrcSection = parsedData.sections ? parsedData.sections.find((s) => s.Name && s.Name.replace(/\0/g, "").toLowerCase() === ".rsrc") : null;

        if (rsrcSection) {
            // 显示资源节基本信息
            const rows = [
                [t("sectionName"), ".rsrc", t("resourceSection")],
                [t("virtualAddress"), formatAddress(rsrcSection.VirtualAddress || 0), t("memoryAddress")],
                [t("virtualSize"), String(rsrcSection.VirtualSize || 0), `${rsrcSection.VirtualSize} ${t("bytes")}`],
                [t("rawDataPointer"), formatAddress(rsrcSection.PointerToRawData || 0), t("fileOffset")],
                [t("rawDataSize"), String(rsrcSection.SizeOfRawData || 0), `${rsrcSection.SizeOfRawData} ${t("bytes")}`],
                [t("characteristics"), `0x${(rsrcSection.Characteristics || 0).toString(16).toUpperCase()}`, t("sectionFlags")],
            ];

            peDetails.appendChild(createTable(t("resourceSectionInfo"), [t("field"), t("value"), t("description")], rows, ["", "pe-details-value", ""]));
        }
        ...
        Object.keys(parsedData.resources)
            ...
            .forEach((typeNum) => {
                ...
                typeRows.push([String(typeId), typeName, String(count), `${entries.reduce((sum, e) => sum + e.size, 0)} ${t("bytes")}`]);
            });

        typeRows.push(["", t("total"), String(totalResources), ""]);

        peDetails.appendChild(createTable(t("parsedResourceTypes"), [t("typeId"), t("name"), t("count"), t("totalSize")], typeRows, ["pe-details-value", "", "pe-details-value", "pe-details-value"]));
    ...
    function showIconInContainerWithSize(entry, iconId, container, logPrefix, width, height, bitCount) {
        try {
            ...
            if (width !== null && height !== null) {
                ...
                infoLabel.innerHTML = `<div style="font-weight: bold; color: var(--vscode-foreground);">${idStr}</div>` + `<div>${width}×${height}</div>` + (bitCount ? `<div>${bitCount}${t("bitSuffix")}</div>` : "");
            } else {
                const idStr = typeof iconId === "string" ? iconId : `#${iconId}`;
    ...
    function showResourceType(resourceType) {
        ...
        const resourceRows = entries.map((entry, index) => {
            ...
            return [String(index + 1), idStr, sizeStr, entry.codePage ? String(entry.codePage) : t("na")];
        });

        peDetails.appendChild(createTable(t("resourceList"), [t("serialNumber"), t("idOrName"), t("size"), t("codePage")], resourceRows, ["pe-details-value", "pe-details-value", "pe-details-value", "pe-details-value"]));
            ...
            entries.forEach((groupEntry, groupIndex) => {
                ...
                try {
                    ...
                    for (let i = 0; i < iconCount && 6 + i * 14 + 14 <= groupBytes.length; i++) {
                        ...
                        const bytesInRes = groupBytes[entryOffset + 8] | (groupBytes[entryOffset + 9] << 8) | (groupBytes[entryOffset + 10] << 16) | (groupBytes[entryOffset + 11] << 24);
                    ...
                    iconInfos.forEach((info, index) => {
                        showIconInContainerWithSize(info.entry, info.iconId, iconContainer, `${groupIndex}-${index}`, info.width, info.height, info.bitCount);
                    });
        ...
        if (typeId === 16 && entries.length > 0) {
            ...
            try {
                ...
                if (versionData) {
                    ...
                    peDetails.appendChild(createTable(t("versionInfoFields"), [t("field"), t("value")], versionRows, ["", "pe-details-value"]));
                }
            } catch (error) {
                const errorText = document.createElement("p");
