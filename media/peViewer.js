...
function buildTree() {
  ...
  if (exportCount) {
    const count = parsedData.exports && parsedData.exports.functions ?
  ...
  if (importsList && parsedData.imports && parsedData.imports.length > 0 &&
  ...
  document.querySelectorAll('.pe-tree-item').forEach((item) => {
    item.addEventListener('click', (e) => {
      ...
      e.stopPropagation();
      const itemId =
          /** @type {string | null} */ (item.getAttribute('data-item'));
...
function showDetails(itemId) {
  ...
  if (itemId.startsWith('imports.')) {
    ...
    if (parts.length === 2) {
      ...
      if (!isNaN(dllIndex) && parsedData.imports &&
...
function showOverview() {
  ...
  let is64Bit = false;
  let bitInfo = t('bit32');
  if (parsedData.nt_headers && parsedData.nt_headers.OptionalHeader &&
  ...
  archHeader.innerHTML = `${t('architectureInfo')}: <span style="color: ${
      is64Bit ?
          '#4CAF50' :
          '#2196F3'}; font-weight: bold;">${bitInfo} ${t('bitPEFile')}</span>`;
  ...
  if (parsedData.dos_header) {
    const dosRows = [
      [
        t('magic'), String(parsedData.dos_header.e_magic || 'N/A'),
        `0x${(parsedData.dos_header.e_magic || 0).toString(16).toUpperCase()}`,
        t('mzIdentifier')
      ],
      [
        t('ntHeaderOffset'), String(parsedData.dos_header.e_lfanew || 'N/A'),
    ...
    container.appendChild(createTable(
        t('dosHeaderInfo'),
        [t('field'), t('value'), t('hex'), t('description')], dosRows,
  ...
  if (parsedData.nt_headers) {
    const ntRows = [];
    ntRows.push([
      '签名', String(parsedData.nt_headers.Signature || 'N/A'),
    ...
    if (parsedData.nt_headers.FileHeader) {
      ntRows.push([
        '机器类型', String(parsedData.nt_headers.FileHeader.Machine || 'N/A'),
        `0x${
            (parsedData.nt_headers.FileHeader.Machine || 0)
                .toString(16)
                .toUpperCase()}`,
        '目标 CPU'
      ]);
    ...
    container.appendChild(createTable(
        t('ntHeaderInfo'), [t('field'), t('value'), t('hex'), t('description')],
        ntRows, ['', 'pe-details-value', 'pe-details-hex', '']));
  }

  // 节信息
  if (parsedData.sections && parsedData.sections.length > 0) {
    const sectionRows = parsedData.sections.map((section, index) => {
      const sectionName = section.Name ? section.Name.replace(/\0/g, '') :
                                         `Section ${index + 1}`;
      return [
        sectionName, String(section.VirtualSize || 'N/A'),
    ...
    container.appendChild(createTable(
        ...
        sectionRows, [
          'pe-details-value', 'pe-details-value', 'pe-details-hex',
          'pe-details-value', 'pe-details-hex', 'pe-details-hex'
        ]));
  }

  // 导出函数统计
  if (parsedData.exports && parsedData.exports.functions &&
  ...
  if (parsedData.imports && parsedData.imports.length > 0) {
    let totalImportFunctions = 0;
    const importDllRows =
        parsedData.imports.map((/** @type {ImportDLL} */ dll) => {
          const funcCount = dll.functions ? dll.functions.length : 0;
          totalImportFunctions += funcCount;
    ...
    container.appendChild(createTable(
        '导入函数统计', ['DLL 名称', '函数数量'], importDllRows,
...
function showCoffHeader() {
  if (!parsedData || !parsedData.nt_headers ||
      !parsedData.nt_headers.FileHeader || !peDetails || !detailsTitle) {
    return;
  }
...
function showOptionalHeader() {
  if (!parsedData || !parsedData.nt_headers ||
      !parsedData.nt_headers.OptionalHeader || !peDetails || !detailsTitle) {
    return;
  }
  ...
  container.appendChild(header);
  container.appendChild(generateValueDetails(
      parsedData.nt_headers.OptionalHeader, 'optional_header'));
...
function showDataDirectory() {
  ...
  const dataDir =
      /** @type {any} */ (parsedData.nt_headers.OptionalHeader).DataDirectory;
  if (!dataDir) {
    return;
  }
  ...
  const directoryNames = [
    t('exportTable'), t('importTable'), t('resourceTable'), t('exceptionTable'),
    t('certificateTable'), t('relocationTable'), t('debugInfo'),
    t('architectureData'), t('globalPointer'), t('tlsTable'),
    t('loadConfigTable'), t('boundImportTable'), t('iat'),
    t('delayImportTable'), t('clrRuntimeHeader'), t('reserved')
  ];
  ...
  dataDir.forEach((/** @type {any} */ dir, /** @type {number} */ index) => {
    if (dir && (dir.VirtualAddress || dir.Size)) {
      dirRows.push([
        String(index), directoryNames[index] || t('unknown'),
        formatAddress(dir.VirtualAddress || 0), String(dir.Size || 0)
      ]);
    }
  });
  ...
  peDetails.appendChild(createTable(
      ...
      [t('index'), t('type'), t('virtualAddress'), t('size')], dirRows, [
        'pe-details-value', 'pe-details-value', 'pe-details-hex',
        'pe-details-value'
      ]));
}
...
function showAllSections() {
  ...
  detailsTitle.textContent = `${t('sectionsList')} (${
      t('totalSections').replace('{count}', parsedData.sections.length)})`;
  peDetails.innerHTML = '';
  ...
  const sectionRows = parsedData.sections.map(
      (/** @type {Section} */ section, /** @type {number} */ index) => {
        const sectionName = section.Name ? section.Name.replace(/\0/g, '') :
                                           `Section ${index + 1}`;
        return [
          sectionName, formatAddress(section.VirtualAddress || 0),
  ...
  peDetails.appendChild(createTable(
      ...
      [
        t('sectionName'), t('virtualAddress'), t('virtualSize'),
        t('rawPointer'), t('rawSize'), t('characteristics')
      ],
      sectionRows, [
        'pe-details-value', 'pe-details-hex', 'pe-details-value',
        'pe-details-hex', 'pe-details-value', 'pe-details-hex'
      ]));
}
...
function showSection(sectionName) {
  ...
  const header = document.createElement('h4');
  header.textContent =
      t('sectionDetails').replace('{sectionName}', sectionName);
  container.appendChild(header);
  container.appendChild(
...
function showExports() {
  ...
  detailsTitle.textContent = `${t('exportFunctions')} (${
      t('totalFunctions').replace('{count}', totalCount)})`;
  peDetails.innerHTML = '';

  // 缓存所有导出行数据
  allExportRows =
      parsedData.exports.functions.map((/** @type {ExportFunction} */ func) => {
        const decodedName = demangleFunctionName(func.name);
        return [
          String(func.ordinal), formatAddress(func.address),
          decodedName,  // 解码后的名称
          func.name,    // 始终显示原始函数名
        ];
      });
...
function renderExportPage() {
  ...
  const tableFragment = createTable(
      ...
      [
        t('ordinal'), t('addressRVA'), t('decodedFunctionName'),
  ...
  const paginationContainer = createPaginationControls(
      currentPage, totalPages, totalCount, startIndex + 1, endIndex);
...
function createPaginationControls(
    page, totalPages, totalCount, startIndex, endIndex) {
  const container = document.createElement('div');
...
function createPageButton(text, enabled, onClick) {
  ...
  button.style.backgroundColor = enabled ?
      ...
      'var(--vscode-button-secondaryBackground)';
  button.style.color = enabled ? 'var(--vscode-button-secondaryForeground)' :
...
function showImportsOverview() {
  ...
  detailsTitle.textContent = `${t('importFunctionsOverview')} (${
      t('importFunctionsCount')
          .replace('{totalFunctions}', totalFunctions)
          .replace('{dllCount}', parsedData.imports.length)})`;
  peDetails.innerHTML = '';

  // 缓存所有导入行数据
  allImportRows = allFunctions.map((func) => [func.dll, func.name, func.type]);
...
function showDllImports(dll) {
  ...
  const funcCount = dll.functions ? dll.functions.length : 0;
  detailsTitle.textContent = `${dll.name} - ${
      t('importedFunctionsTitle').replace('{dllName}', dll.name)} (${
      t('importedFunctionsCount').replace('{count}', funcCount)})`;
  peDetails.innerHTML = '';
  ...
  allImportRows = dll.functions.map((/** @type {ImportFunction} */ func) => {
    return [
      dll.name, func.name || `${t('ordinalPrefix')}${func.ordinal}`,
...
function renderImportPage() {
  ...
  const paginationContainer = createImportPaginationControls(
      currentImportPage, totalPages, totalCount, startIndex + 1, endIndex);
  peDetails.appendChild(paginationContainer);
}
...
function generateValueDetails(value, path) {
  ...
  if (typeof value === 'number' || typeof value === 'bigint') {
    ...
    container.appendChild(createTable(
        t('numericDetails'), [t('property'), t('value')], rows,
        ['', 'pe-details-value']));
    ...
    container.appendChild(createTable(
        t('stringDetails'), [t('property'), t('value')], rows,
        ['', 'pe-details-value']));
    ...
    for (const [key, val] of Object.entries(value)) {
      if (typeof val === 'number' || typeof val === 'bigint') {
        ...
        rows.push(
            [key, String(numVal), `0x${numVal.toString(16).toUpperCase()}`]);
      } else if (typeof val === 'string') {
        rows.push([key, val, '-']);
      } else if (Array.isArray(val)) {
        rows.push(
            [key, t('arrayWithLength').replace('{length}', val.length), '-']);
    ...
    container.appendChild(createTable(
        t('structureDetails'), [t('field'), t('value'), t('hexadecimal')], rows,
        ['', 'pe-details-value', 'pe-details-hex']));
  } else {
...
function showResourcesOverview() {
  ...
  if (rsrcSection) {
    ...
    const rows = [
      ...
      [
        t('virtualAddress'), formatAddress(rsrcSection.VirtualAddress || 0),
        t('memoryAddress')
      ],
      [
        t('virtualSize'), String(rsrcSection.VirtualSize || 0),
        `${rsrcSection.VirtualSize} ${t('bytes')}`
      ],
      [
        t('rawDataPointer'), formatAddress(rsrcSection.PointerToRawData || 0),
        t('fileOffset')
      ],
      [
        t('rawDataSize'), String(rsrcSection.SizeOfRawData || 0),
    ...
    peDetails.appendChild(createTable(
        t('resourceSectionInfo'), [t('field'), t('value'), t('description')],
        rows, ['', 'pe-details-value', '']));
  }
  ...
  Object.keys(parsedData.resources)
      ...
      .forEach((typeNum) => {
        ...
        typeRows.push([
          String(typeId), typeName, String(count),
  ...
  peDetails.appendChild(createTable(
      ...
      [t('typeId'), t('name'), t('count'), t('totalSize')], typeRows,
      ['pe-details-value', '', 'pe-details-value', 'pe-details-value']));
...
function showIconInContainer(entry, iconId, container, logPrefix) {
  showIconInContainerWithSize(
      entry, iconId, container, logPrefix, null, null, null);
}
...
function showIconInContainerWithSize(
    entry, iconId, container, logPrefix, width, height, bitCount) {
  try {
    ...
    // 检查是否是PNG格式
    if (iconData.length > 4 && iconData[0] === 0x89 && iconData[1] === 0x50 &&
      ...
      const headerSize = iconData[0] | (iconData[1] << 8) |
          (iconData[2] << 16) | (iconData[3] << 24);

      if (headerSize === 40) {
        const width = iconData[4] | (iconData[5] << 8) | (iconData[6] << 16) |
            (iconData[7] << 24);
        const fullHeight = iconData[8] | (iconData[9] << 8) |
            (iconData[10] << 16) | (iconData[11] << 24);
    ...
    img.onload = () => {
      console.log(`[Icon ${logPrefix}] ✓ Loaded: ${img.naturalWidth}x${
          img.naturalHeight}`);
    };
...
function showResourceType(resourceType) {
  ...
  if (!parsedData.resources || !parsedData.resources[typeId]) {
    showEmptyMessage(`${t('noResourcesFound').toLowerCase()} ${resInfo.name}.`);
    return;
  }
  ...
  if (typeId === 2 && entries.length > 0) {
    ...
    entries.forEach((entry, index) => {
      try {
        ...
        const headerSize = dibData[0] | (dibData[1] << 8) | (dibData[2] << 16) |
            (dibData[3] << 24);
        const width = dibData[4] | (dibData[5] << 8) | (dibData[6] << 16) |
            (dibData[7] << 24);
        const height = dibData[8] | (dibData[9] << 8) | (dibData[10] << 16) |
            (dibData[11] << 24);
        ...
        let paletteSize = 0;
        if (bitCount <= 8) {
          const colorsUsed = dibData[32] | (dibData[33] << 8) |
              (dibData[34] << 16) | (dibData[35] << 24);
        ...
        infoDiv.style.color = 'var(--vscode-descriptionForeground)';
        infoDiv.textContent =
            `位图 ${bitmapId} (${width}x${Math.abs(height)}, ${bitCount}bit)`;
  ...
  if (typeId === 14 && entries.length > 0) {
    ...
    entries.forEach((groupEntry, groupIndex) => {
      ...
      const groupId = typeof groupEntry.id === 'string' ? groupEntry.id :
                                                          `#${groupEntry.id}`;
      ...
      try {
        ...
        for (let i = 0; i < iconCount && 6 + i * 14 + 14 <= groupBytes.length;
             i++) {
          ...
          const bytesInRes = groupBytes[entryOffset + 8] |
              (groupBytes[entryOffset + 9] << 8) |
              (groupBytes[entryOffset + 10] << 16) |
              (groupBytes[entryOffset + 11] << 24);
          const iconId = groupBytes[entryOffset + 12] |
              (groupBytes[entryOffset + 13] << 8);

          console.log(`[Icon Group ${groupIndex}] Entry ${i}: iconId=${
              iconId}, size=${width}x${height}, bits=${bitCount}`);
        ...
        iconInfos.forEach((info, index) => {
          showIconInContainerWithSize(
              info.entry, info.iconId, iconContainer, `${groupIndex}-${index}`,
              info.width, info.height, info.bitCount);
        });
  ...
  if (typeId === 6 && entries.length > 0) {
    ...
    try {
      ...
      entries.forEach((entry, entryIndex) => {
        const blockId = typeof entry.id === 'number' ?
            entry.id :
        ...
        try {
          ...
          console.warn(
              `Failed to parse string table entry ${entryIndex}:`, error);
        }
      });
  ...
  if (typeId === 16 && entries.length > 0) {
    ...
    try {
      ...
      if (versionData) {
        const versionRows =
            Object.entries(versionData).map(([key,
                                              value]) => [key, String(value)]);
        peDetails.appendChild(createTable(
            t('versionInfoFields'), [t('field'), t('value')], versionRows,
            ['', 'pe-details-value']));
      }
    } catch (error) {
      const errorText = document.createElement('p');
  ...
  try {
    ...
    for (let i = 0; i < 16 && offset < dataArray.length; i++) {
      ...
      if (offset + strByteLen > dataArray.length) {
        console.warn(`String ${baseStringId + i}: not enough data (need ${
            strByteLen}, have ${dataArray.length - offset})`);
        break;
      }
    ...
    fields.forEach((field) => {
      ...
      if (index !== -1) {
        ...
        while (valueStart < text.length &&
               (text[valueStart] === '\0' || text[valueStart] === ' ' ||
                text[valueStart] === '\t')) {
          valueStart++;
        }
        ...
        const value =
            text.substring(valueStart, valueEnd).replace(/\0/g, '').trim();
        if (value) {
