...
- ⇄ **符号对比功能**：支持将当前打开的文件与另一个同类型文件进行导出符号对比
- 📊 **差异可视化**：清晰展示"仅在文件 A 中"、"仅在文件 B 中"和"共同符号"三类差异
- 🔍 **对比结果搜索**：每个差异分类下均支持符号名实时搜索过滤
- 📄 **分页浏览**：大量差异符号自动分页展示，每页 200 条
- 🌐 **跨格式对比**：支持 PE、ELF、LIB 三种格式之间的任意对比

#### 界面改进

- 🔘 详情面板头部新增「⇄ 对比符号」按钮，一键发起对比
- 🃏 对比结果以文件信息卡片 + 统计标签 + Tab 切换列表呈现
- 🔙 对比视图中提供"返回查看器"按钮，快速切回正常浏览模式

#### 解析体验优化

- ⏳ **解析中弹窗提示**：文件尚未解析完成时显示 loading 遮罩，解析完成后自动消失

### 技术改进

- 🔄 对比文件解析复用 `BinaryFileDocument.create`，确保与主文件解析结果一致
- 📡 基于 webview 双向消息机制实现对比流程，无需新建额外面板
- 🌐 中英文国际化完整覆盖对比功能所有文本
