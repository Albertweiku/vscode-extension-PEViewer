...
class PEDocument extends Disposable implements vscode.CustomDocument {
    static async create(uri: vscode.Uri, backupId: string | undefined, delegate: PEDocumentDelegate): Promise<PEDocument> {
    ...
    private static async parseExtendedData(fileData: Buffer, basicData: any): Promise<ExtendedPEData> {
        ...
        try {
            // 解析资源
            extendedData.resources = PEDocument.parseResourceDirectory(fileData, basicData);
        } catch (error) {
    ...
    private static parseImportTable(fileData: Buffer, basicData: any): ImportDLL[] {
        ...
        // 将RVA转换为文件偏移
        const importTableOffset = PEDocument.rvaToOffset(importTableEntry.VirtualAddress, basicData.sections);
        ...
        while (offset < fileData.length) {
            ...
            // 如果所有字段都是0，表示导入表结束
            if (importLookupTableRVA === 0 && timeDateStamp === 0 && forwarderChain === 0 && nameRVA === 0 && importAddressTableRVA === 0) {
                break;
            }
            ...
            const lookupTableRVA = importLookupTableRVA !== 0 ? importLookupTableRVA : importAddressTableRVA;
            let lookupOffset = PEDocument.rvaToOffset(lookupTableRVA, basicData.sections);
            ...
            while (lookupOffset + entryIndex * entrySize < fileData.length) {
                ...
                if (is64Bit) {
                    // 64位：使用BigInt处理
                    const entry64 = fileData.readBigUInt64LE(lookupOffset + entryIndex * entrySize);
                    ...
                    if (isOrdinal) {
                        ...
                        if (entry64 > rvaMaxValue) {
                            console.warn(`Invalid RVA value in 64-bit import table: ${entry64.toString(16)}`);
                            break;
                        }
                        rvaValue = Number(entry64);
                        const hintNameOffset = PEDocument.rvaToOffset(rvaValue, basicData.sections);
                        if (hintNameOffset < 0 || hintNameOffset >= fileData.length - 2) {
                            console.warn(`Invalid hint/name offset: ${hintNameOffset} (RVA: 0x${rvaValue.toString(16)})`);
                            break;
                        }
                    ...
                    // 32位：使用Number处理
                    const entry32 = fileData.readUInt32LE(lookupOffset + entryIndex * entrySize);
                    ...
                    if (isOrdinal) {
                        ...
                        // 按名称导入
                        const hintNameOffset = PEDocument.rvaToOffset(entry32, basicData.sections);
                        if (hintNameOffset < 0 || hintNameOffset >= fileData.length - 2) {
                            break;
                        }
                        const hint = fileData.readUInt16LE(hintNameOffset);
    ...
    private static parseExportTable(fileData: Buffer, basicData: any): ExportTable | undefined {
        if (!basicData.nt_headers?.OptionalHeader?.DataDirectory) {
            return undefined;
        }
        ...
        const exportTableOffset = PEDocument.rvaToOffset(exportTableEntry.VirtualAddress, basicData.sections);
        ...
        // 读取函数地址表
        const functionsOffset = PEDocument.rvaToOffset(addressOfFunctions, basicData.sections);
        ...
        // 读取名称表和序号表
        const namesOffset = PEDocument.rvaToOffset(addressOfNames, basicData.sections);
        const ordinalsOffset = PEDocument.rvaToOffset(addressOfNameOrdinals, basicData.sections);
        ...
        for (let i = 0; i < numberOfNames; i++) {
            ...
            // 读取函数名称
            const funcNameOffset = PEDocument.rvaToOffset(nameRVA, basicData.sections);
            let funcName = "";
    ...
    private static parseResourceDirectory(fileData: Buffer, basicData: any): ResourceDirectory | undefined {
        if (!basicData.nt_headers?.OptionalHeader?.DataDirectory) {
            return undefined;
        }
        ...
        const resourceOffset = PEDocument.rvaToOffset(resourceRVA, basicData.sections);
        ...
        try {
            // 解析资源目录表（第一层：类型）
            PEDocument.parseResourceLevel(fileData, resourceOffset, resourceRVA, resourceOffset, 0, resources, null, null, basicData.sections);
        } catch (error) {
    ...
    private static parseResourceLevel(fileData: Buffer, offset: number, resourceBaseRVA: number, resourceBaseOffset: number, level: number, resources: ResourceDirectory, typeId: number | null, nameId: number | string | null, sections: any[]): void {
        ...
        for (let i = 0; i < totalEntries; i++) {
            ...
            if (isNamedEntry) {
                ...
                if (nameOffset + 2 <= fileData.length) {
                    ...
                    let name = "";
                    for (let j = 0; j < nameLength && nameOffset + 2 + j * 2 + 1 < fileData.length; j++) {
            ...
            if (isDirectory) {
                ...
                if (level === 0) {
                    // 第一层：类型
                    PEDocument.parseResourceLevel(fileData, subdirOffset, resourceBaseRVA, resourceBaseOffset, level + 1, resources, typeof entryId === "number" ? entryId : 0, null, sections);
                } else if (level === 1) {
                    // 第二层：ID/名称
                    PEDocument.parseResourceLevel(fileData, subdirOffset, resourceBaseRVA, resourceBaseOffset, level + 1, resources, typeId, entryId, sections);
                } else {
                    // 第三层：语言（直接指向数据）
                    PEDocument.parseResourceLevel(fileData, subdirOffset, resourceBaseRVA, resourceBaseOffset, level + 1, resources, typeId, nameId, sections);
                }
            } else {
                // 数据条目
...
export class PEEditorProvider implements vscode.CustomEditorProvider<PEDocument> {
    ...
    public static register(context: vscode.ExtensionContext): vscode.Disposable {
        return vscode.window.registerCustomEditorProvider(PEEditorProvider.viewType, new PEEditorProvider(context), {
            webviewOptions: {
    ...
    async resolveCustomEditor(document: PEDocument, webviewPanel: vscode.WebviewPanel, _token: vscode.CancellationToken): Promise<void> {
    ...
    private readonly _onDidChangeCustomDocument = new vscode.EventEmitter<vscode.CustomDocumentEditEvent<PEDocument>>();
    public readonly onDidChangeCustomDocument = this._onDidChangeCustomDocument.event;

    public saveCustomDocument(document: PEDocument, cancellation: vscode.CancellationToken): Thenable<void> {
        return this.doSave(document, document.uri, cancellation);
    }

    public saveCustomDocumentAs(document: PEDocument, destination: vscode.Uri, cancellation: vscode.CancellationToken): Thenable<void> {
        return this.doSave(document, destination, cancellation);
    }

    public revertCustomDocument(document: PEDocument, cancellation: vscode.CancellationToken): Thenable<void> {
        return PEDocument.loadData(document.uri).then(async (data) => {
            ...
            this.postMessageToAll(document.uri, "update", {
                parsedData: JSON.parse(JSON.stringify(document.parsedData, (key, value) => (typeof value === "bigint" ? value.toString() : value))),
            });
        });
    }

    public backupCustomDocument(document: PEDocument, context: vscode.CustomDocumentBackupContext, cancellation: vscode.CancellationToken): Thenable<vscode.CustomDocumentBackup> {
    ...
    private async doSave(document: PEDocument, destination: vscode.Uri, cancellation: vscode.CancellationToken): Promise<void> {
    ...
    private getHtmlForWebview(webview: vscode.Webview): string {
        ...
        const styleMainUri = webview.asWebviewUri(vscode.Uri.joinPath(this._context.extensionUri, "media", "peViewer.css"));

        const htmlUri = webview.asWebviewUri(vscode.Uri.joinPath(this._context.extensionUri, "media", "peViewer.html"));

        // Use a nonce to whitelist which scripts can be run
        const nonce = getNonce();

        // Read HTML template
        const htmlContent = fs.readFileSync(vscode.Uri.joinPath(this._context.extensionUri, "media", "peViewer.html").fsPath, "utf8");
